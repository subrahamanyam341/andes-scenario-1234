...
import (
	...
	exporter "github.com/subrahamanyam341/andes-scenario-1234/scenario/exporter"
...
func TestGetAccountsAndTransactionsFrom_Adder(t *testing.T) {
	...
	transaction := exporter.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*scenmodel.DCTTxData, 0), sbi.Accs[0].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
...
func TestGetAccountsAndTransactionsFrom_AdderWithExternalSteps(t *testing.T) {
	...
	transactionAlice := exporter.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*scenmodel.DCTTxData, 0), sbi.Accs[0].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
	transactionBob := exporter.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*scenmodel.DCTTxData, 0), sbi.Accs[2].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
	transactionOwner := exporter.CreateTransaction("add", [][]byte{{3}}, 0, big.NewInt(0), make([]*scenmodel.DCTTxData, 0), sbi.Accs[3].GetAddress(), sbi.Accs[1].GetAddress(), 5000000, 1)
